...
- `pnpm dev` → standalone harness (`dev/`) on port 5180: imports `src/` directly for
  instant HMR, renders the chart on committed real NSE bars in a trading-app-like frame.
...
`dev/` is a **tracked** standalone Vite app (`pnpm dev`) that renders chart-core's own
public components on committed real bars — entry `dev/App.tsx`, layout formula in
`dev/layout.ts`, per-symbol data under `dev/public/eod/` (regenerated by
`dev/tools/extract_eod_sample.py`). It ships to nobody: `files:["dist"]` and the `dist`
publish step exclude it. Kept separate from `e2e/fixture/` so its chrome can't disturb
the Playwright behaviour locks.
