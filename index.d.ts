...
export declare function barIndexForDateProjected(data: readonly Candle[], isoDate: string): number;
 ...
 */
export declare function barsPerYear(data: readonly {
    date: string;
}[]): number;
...
export declare const CANDLE_SOURCE = "__candles__";
 ...
 */
export declare type CandleAppearance = {
    /** 0..1, applied as `globalAlpha` around the price-series paint pass. */
    opacity: number;
};

export declare const Chart: default_2.MemoExoticComponent<({ data, warmupSeed, benchmarkClose, quarterlyResults, subpaneHeights, onSubpaneHeightsChange, visibleBars, onVisibleBarsChange, onMaxVisibleBarsChange, onRangeMarksChange, panOffset, onPanOffsetChange, chartType, indicators, onIndicatorsChange, autoFitMode, onAutoFitModeChange, autoFitExcluded, onAutoFitExcludedChange, infoBarExpanded, onInfoBarExpandedChange, symbol, bare, width: propWidth, height: propHeight, priceFormatter, patterns, patternsEnabled, visiblePatterns, statsTable, statsEnabled, statsMarket, statsPosition, onStatsPositionChange, statsSize, earningsEnabled, earningsResults, earningsFreeFloatPercent, earningsPosition, onEarningsPositionChange, appearance, onAppearanceChange, drawings, onDrawingsChange, activeDrawingTool, onActiveDrawingToolChange, drawToolbarEnabled, drawToolbarPosition, onDrawToolbarPositionChange, onContextMenu, children, }: Props_4) => default_2.JSX.Element>;
...
export declare type ChartContextMenuInfo = {
    clientX: number;
    clientY: number;
...
export declare function ChartControls({ chartType, onChartTypeChange, indicators, onIndicatorsChange, patternsEnabled, onPatternsToggle, visiblePatterns, onVisiblePatternsChange, statsEnabled, onStatsToggle, earningsEnabled, onEarningsToggle, className, }: Props): JSX.Element;
...
export declare function dateForBarIndexProjected(data: readonly Candle[], idx: number): string;
...
export declare const DEFAULT_BARS_PER_YEAR = 252;

/** The legacy daily ladder as marks — default for `marks`-less consumers. */
export declare const DEFAULT_RANGE_MARKS: RangeMark[];
...
export declare const DRAWING_DEFAULTS: {
    readonly color: "var(--chart-drawing)";
    readonly width: 1.5;
    readonly style: 0;
    readonly opacity: 1;
    readonly text: "";
    readonly fontSize: 12;
    readonly bgColor: "var(--chart-drawing-bg)";
    readonly bgOpacity: 0.85;
};

export declare type DrawingAnchor = {
    date: string;
    price: number;
};

declare type DrawingBase = {
    id: string;
    locked?: boolean;
    style?: DrawingStyle;
    v?: number;
...
export declare type DrawingShape = TrendLineDrawing | HorizontalLineDrawing | VerticalLineDrawing | HorizontalRayDrawing | RayDrawing | TextDrawing | RulerDrawing;

export declare type DrawingStyle = {
    color?: string;
    width?: number;
    style?: number;
    opacity?: number;
...
export declare type DrawingTool = DrawingType | 'cursor';

export declare type DrawingType = DrawingShape['type'];
...
export declare type EarningsCell = {
    text: string;
    level: StatsLevel;
};
...
export declare type EarningsRow = {
    label: string;
    cells: EarningsCell[];
};
...
export declare type EarningsViewModel = {
    rows: EarningsRow[];
    freeFloat: EarningsCell;
};
...
declare type EffectiveDrawingStyle = {
    color: string;
    width: number;
    style: number;
    opacity: number;
...
export declare function effectiveDrawingStyle(style?: DrawingStyle): EffectiveDrawingStyle;
...
export declare const FILLED_HIT_PAD = 2;
...
export declare type HitRegionSink = {
    add(region: HitRegionSpec): void;
};

/** What a painter hands the sink — the framework stamps `sourceId`. */
export declare type HitRegionSpec = Omit<HitRegion, 'sourceId'>;

export declare type HorizontalLineDrawing = DrawingBase & {
    type: 'hline';
    price: number;
};

export declare type HorizontalRayDrawing = DrawingBase & {
    type: 'hray';
    a: DrawingAnchor;
};
...
export declare type IndicatorDef<S = Record<string, unknown>> = {
    ...
    draw(ctx: CanvasRenderingContext2D, series: IndicatorSeries, scale: IndicatorDrawScale, s: S, resolveColor: (expr: string, prop?: string) => string, meta?: unknown): void;
...
export declare type LegacyStatsPosition = {
    x: number;
    y: number;
};
...
export declare const MIN_MARK_BARS = 30;
...
export declare function normalizeDrawing(raw: unknown): DrawingShape | null;
 ...
 * cannot draw. */
export declare function normalizeStatsPosition(raw: unknown): StatsPosition | LegacyStatsPosition | null;
 ...
 */
export declare function pickHitRegion(mx: number, my: number, barIdx: number, regions: readonly HitRegion[], centerXAt: (g: number) => number, step: number, tolerance?: number): HitRegion | null;
 ...
 */
export declare const RANGE_DAYS: Record<'3M' | '6M' | '1Y' | '2Y' | '3Y' | '5Y', number>;

/** Named range → calendar years. Interval-agnostic: bars/year is measured. */
export declare const RANGE_YEARS: Record<RangeKey_2, number>;

export declare type RangeKey = '3M' | '6M' | '1Y' | '2Y' | '3Y' | '5Y' | '10Y' | '20Y';

declare type RangeKey_2 = '3M' | '6M' | '1Y' | '2Y' | '3Y' | '5Y' | '10Y' | '20Y';
 ...
 */
export declare function rangeMarks(data: readonly {
    date: string;
}[], dataLength: number): RangeMark[];
...
export declare type RayDrawing = DrawingBase & {
    type: 'ray';
    a: DrawingAnchor;
    b: DrawingAnchor;
};
...
export declare const REGION_HIT_TOLERANCE = 6;
...
export declare type RulerDrawing = DrawingBase & {
    type: 'ruler';
    a: DrawingAnchor;
    b: DrawingAnchor;
};

export declare function SettingsDialog({ appearance, onAppearanceChange, resolveColor, onClose, triggerRef, style, }: Props_2): JSX.Element;
...
declare type StatsLevel = 'strong' | 'up' | 'neutral' | 'down' | 'text' | 'muted';

export declare type StatsMarket = 'India' | 'US';

export declare type StatsPaneRect = {
    left: number;
    top: number;
    width: number;
    height: number;
...
export declare type TextDrawing = DrawingBase & {
    type: 'text';
    a: DrawingAnchor;
};
...
export declare type TrendLineDrawing = DrawingBase & {
    type: 'trendline';
    a: DrawingAnchor;
    b: DrawingAnchor;
};
...
export declare type VerticalLineDrawing = DrawingBase & {
    type: 'vline';
    date: string;
};
...
export declare function ZoomSlider({ visibleBars, onVisibleBarsChange, maxVisibleBars, marks: marksProp, onPanReset, }: Props_3): JSX.Element;
